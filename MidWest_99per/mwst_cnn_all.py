...
def create_model(channels):
    model = Sequential()
    #add model layers
    model.add(Conv2D(16, kernel_size=3, activation='relu', input_shape=(20,19,channels), activity_regularizer=l2(0.001)))
...
def cnn_combo(numofvar):
    ...
    file1 = open("mwst_cnn_all_results.txt", "w")
    ...
    for i in list(comb):
        ...
        for i in range(100):
            ...
            my_X_train = train_x.reshape(train_x.shape[0],20,19,channels)
            my_X_test = test_x.reshape(1288,20,19,channels)
            my_y_train = train_y.reshape(train_x.shape[0],)
            my_y_test = test_y.reshape(1288,)
